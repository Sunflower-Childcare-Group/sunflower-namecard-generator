...
class NamecardGenerator {
    ...
    async renderNamecard(data) {
        ...
        // Mobile/Office numbers - dynamically positioned
        const phoneNumbers = [];
        ...
        // Address - dynamically positioned so last line aligns with QR code bottom
        if (data.officeAddress) {
    ...
    async renderVectorPDF(pdf, data, cardWidth, cardHeight) {
        ...
        pdf.setFillColor(251, 201, 22); // RGB equivalent that converts to correct CMYK
    ...
    addVectorText(pdf, data) {
        ...
        // Set text color using RGB equivalent of CMYK(34%, 24%, 0%, 73%)
        // RGB(45, 52, 68) = #2d3444 converts to CMYK(34%, 24%, 0%, 73%) in PDF color space
        pdf.setTextColor(45, 52, 68);
        ...
        // Reapply text color after font change
        pdf.setTextColor(45, 52, 68); // RGB(45, 52, 68) = CMYK(34%, 24%, 0%, 73%)
